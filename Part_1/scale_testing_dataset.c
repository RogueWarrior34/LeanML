...
lml_mat_double *minMaxMatrix;
lml_mat_double *outputMatrix;
dimension_t inputMatrixRows;
dimension_t inputMatrixColumns;
if(argc!=6)
{
printf("Usage: [scale_testing_dataset.out input_file_name output_file_name from_column to_column min_max_file]\n");
return 0;
}
...
minMaxMatrix=lml_mat_double_from_csv(minMaxFile);
if(minMaxMatrix==NULL)
{
printf("Error in loading min_max_file.\n");
lml_mat_double_destroy(inputMatrix);
return 0;
}
outputMatrix=lml_scale_double_given_min_max(inputMatrix,0,fromColumn,inputMatrixRows-1,toColumn,minMaxMatrix);
...
lml_mat_double_destroy(inputMatrix);
lml_mat_double_destroy(minMaxMatrix);
